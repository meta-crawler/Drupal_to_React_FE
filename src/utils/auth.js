...
export function getAuthClient(config = {}) {
    ...
    async function tokenCurrent() {
        const token = localStorage.getItem(config.token_name) !== null ? JSON.parse(localStorage.getItem(config.token_name)) : false;
    ...
    function tokenRefresh(refreshToken) {
        ...
        return (refreshPromises[refreshToken] = fetch(`${config.base}/oauth/token`, {
            method: 'post',
            headers: new Headers({
                Accept: 'application/json'
            }),
